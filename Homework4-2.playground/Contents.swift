...
func listPass(names: [String], scores: [Int?], condition: (Int) -> Bool){
    ...
    for i in scores{
        if let score = i {
            if condition(score){
                print(names[scores.index(of: score)!] + String(score))
            }
        }
    }
}
...
let sum = scoresWithNoOptionals.reduce(0,+)

print("------------------")
// 배열의 메소드만 사용해서 응시자의 총 점수와 평균을 구함

print("응시자 총 점수: " + String(sum))
print("응시자 평균 점수: " + String(Double(sum)/Double(scoresWithNoOptionals.count)))
